...
nodeRouter.post("/", validateRequest<CreateNodeRecord>(nodeSchema), async (req, res) => {
  try {
    const data = await createNode(req.body);
    res.json({ message: "Node created successfully!", data });
  } catch (err: any) {
...
nodeRouter.patch("/:id", validateRequest<UpdateNodeRecord>(updateNodeSchema), async (req, res) => {
  try {
    const data = await updateNode(req.params.id, req.body);
    res.json({ message: "Node updated successfully!", data });
  } catch (err: any) {
    res.status(500).json({ error: err.message });
  }
});
